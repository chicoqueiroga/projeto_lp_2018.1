...
def analiseDePareto(coluna, ano, df):
    ...
    dfAnalisado = dfAnalisado.append(
        {coluna: 'Outros', 'qtdOcorrencia': outrosDf.qtdOcorrencia.sum(), 'porcentagemAcumulada': 100},
        ignore_index=True)
    dfAnalisado.set_index(coluna, inplace=True)

    barras = dfAnalisado.qtdOcorrencia.plot(kind='bar', color='r')
    linha = dfAnalisado.porcentagemAcumulada.plot(linestyle='-', marker='o', ax=barras, secondary_y=True,
                                                  color='#008000', rot=90)
...
def representacaoAssuntoProblema(df):
    ...
    agrupamento = df[df.GrupoAssunto.isin(principaisProdutosServicos)].groupby('GrupoAssunto')[
        'GrupoProblema'].value_counts()
...
agrupamentos = [(agrupamentos[0], agrupamentos[1]), (agrupamentos[1], agrupamentos[2]),
                (agrupamentos[2], agrupamentos[3])]
